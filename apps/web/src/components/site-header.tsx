...
function LanguageSwitcher({ className }: { className?: string }) {
  ...
  return (
    <div ref={ref} className={`relative ${className || ''}`}>
      ...
      {isOpen && (
        <div className="absolute top-full right-0 z-50 mt-1 min-w-[120px] rounded-md border bg-white py-1 shadow-lg">
          {SUPPORTED_LANGUAGES.map((lang) => (
            <button
              key={lang}
              onClick={() => switchLocale(lang)}
              className={`w-full px-3 py-1.5 text-left text-sm hover:bg-coral-soft ${lang === locale ? 'font-bold text-coral' : ''}`}
            >
...
export function SiteHeader() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full bg-white dark:bg-background/80">
      <div className="flex h-14 w-full max-w-none items-center justify-between px-6 md:px-8">
        ...
        <div className="hidden items-center md:flex">
          <Link href="/" className="mr-6 flex items-center space-x-2">
            <Image
              ...
              priority
            />
            <span className="hidden font-playful text-3xl font-bold text-ink sm:inline-block dark:text-white">
        ...
        <div className="flex items-center md:hidden">
          <Link href="/" className="flex items-center space-x-2">
            <Image
              ...
              priority
            />
            <span className="font-playful text-2xl font-bold text-ink sm:inline-block dark:text-white">
              Storywin<span className="text-coral">k.ai</span>
            </span>
          </Link>
        </div>
      ...
      {isMobileMenuOpen && (
        <div
          ref={mobileMenuRef}
          className="absolute top-14 right-0 left-0 z-40 bg-white shadow-md md:hidden dark:bg-background"
        >
          <nav className="container flex flex-col space-y-2 p-4">
            <SignedIn>
              <Button asChild className="group w-fit font-playful">
              ...
              {process.env.NEXT_PUBLIC_AVATARS_ENABLED === 'true' && (
                <Button asChild variant="outline" className="w-fit font-playful">
      ...
      <ScallopEdge
        className={`absolute right-0 left-0 z-30 ${isMobileMenuOpen ? 'max-md:hidden' : ''}`}
        style={{ bottom: '-20px' }}
      />
    </header>
  );
