    ...
    const renderDisplayPage = (dp: DisplayPage<Page>, index: number) => {
      ...
      if (dp.type === 'story') {
        ...
        const artSize =
          textH != null
            ? Math.max(
                Math.round(pageHeight * 0.45),
                Math.min(pageWidth, pageHeight - textH - STRIP_PAD),
              )
        ...
        return (
          <div key={pageKey} className="overflow-hidden rounded-lg border border-black/15 bg-white">
            <div className="absolute inset-0 flex flex-col">
              ...
              <div className="flex min-h-0 flex-1 [touch-action:pan-y] justify-center overflow-y-auto [overscroll-behavior:contain] px-[8%]">
                {dp.page.text && dp.page.text.trim() && (
                  <p
                    className={`${storyFontClass} my-auto py-2 text-center leading-snug text-[#1a1a1a]`}
            ...
            {storyMeasurePages.map((dp) => {
              ...
              return (
                <p
                  ...
                  className={`${measureFontClass} text-center leading-snug text-[#1a1a1a]`}
