...
export async function handleImageUpload(params: UploadFileParams): Promise<UploadResult> {
  ...
  logger.info(
    { userId: dbUser.id, clerkId, fileName, fileType, fileSize, folder },
  ...
  if (!ACCEPTED_FILE_TYPES.includes(fileType)) {
    ...
    return {
      success: false,
      message: `Invalid file type: ${fileType}. Accepted types: ${ACCEPTED_FILE_TYPES.join(', ')}`,
    };
  }
  ...
  if (fileSize > MAX_FILE_SIZE_BYTES) {
    logger.warn(
      { userId: dbUser.id, fileName, fileSize },
  ...
  try {
    ...
    logger.info(
      { userId: dbUser.id, publicId: cloudinaryResult.public_id },
    ...
    logger.info(
      { userId: dbUser.id, assetId: asset.id, publicId: cloudinaryResult.public_id },
