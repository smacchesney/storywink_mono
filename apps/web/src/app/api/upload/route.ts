...
export async function POST(request: Request) {
  try {
    ...
    logger.info(
      { clerkUserId: clerkId, dbUserId: dbUser.id, email: primaryEmail },
      'User authentication completed in upload route.',
    );
    ...
    for (const file of files) {
      ...
      try {
        ...
        console.log(
          `>>> DEBUG: Successfully uploaded ${file.name}, URL: ${cloudinaryResult.secure_url}`,
        );
      } catch (cloudinaryError) {
        console.error(`>>> DEBUG: Cloudinary upload error for ${file.name}:`, cloudinaryError);
        throw new Error(
          `Failed to upload ${file.name} to Cloudinary: ${cloudinaryError instanceof Error ? cloudinaryError.message : 'Unknown error'}`,
        );
      }
      ...
      try {
        ...
        console.log(
          `>>> DEBUG: Added asset to uploadedAssets array, total: ${uploadedAssets.length}`,
        );
      } catch (dbError) {
        console.error(`>>> DEBUG: Database transaction failed for ${file.name}:`, dbError);
        throw new Error(
          `Database transaction failed for ${file.name}: ${dbError instanceof Error ? dbError.message : 'Unknown error'}`,
        );
      }
      // --- End Transaction ---
    }
    ...
    console.log(
      `>>> DEBUG: Finished processing all files. Total uploaded: ${uploadedAssets.length}`,
    );
    ...
    if (uploadedAssets.length === 1) {
      ...
      console.log(
        `>>> DEBUG: WARNING: Multiple files in single request (${uploadedAssets.length}), returning array format`,
      );
      return NextResponse.json(
        {
          success: true,
