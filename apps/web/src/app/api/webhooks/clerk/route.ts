...
export async function POST(req: Request) {
  ...
  try {
    evt = wh.verify(JSON.stringify(payload), {
      // Verify the stringified payload
      'svix-id': svix_id,
  ...
  try {
    switch (eventType) {
      ...
      case 'user.updated':
        ...
        if (!primaryEmail) {
          logger.warn(
            { clerkUserId: id, eventType },
            'User created/updated webhook missing primary email address.',
          );
          ...
          return NextResponse.json(
            { error: 'Missing primary email for user.created/updated event' },
            { status: 400 },
          );
        }

        logger.info(
          { clerkUserId: id, email: primaryEmail, eventType },
          `Calling ensureUser for user...`,
        );
        await ensureUser({
      ...
      case 'user.deleted':
        ...
        if (deleted && deletedClerkId) {
          // Ensure it's a true deletion event and ID is present
          logger.info(
            { clerkUserId: deletedClerkId },
            `Attempting to delete user based on clerkId...`,
          );
          ...
          const cloudinaryTargets = dbUser ? await collectUserCloudinaryTargets(dbUser.id) : null;
          ...
          if (deleteResult.count > 0) {
                ...
                logger.info(
                  {
                    clerkUserId: deletedClerkId,
                    dbUserId: dbUser.id,
                    publicIdCount: cloudinaryTargets.publicIds.length,
                  },
            ...
            logger.warn(
              { clerkUserId: deletedClerkId },
              `User deletion webhook (clerkId) received, but user not found in DB.`,
            );
          }
        } else if (deletedClerkId) {
          logger.warn(
            { clerkUserId: deletedClerkId, deletedStatus: deleted },
            'User deleted webhook received, but deleted status is not true or ID is missing.',
          );
        } else {
