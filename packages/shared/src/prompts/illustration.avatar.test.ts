...
describe('createIllustrationPrompt — sheet anchor (avatar story pages)', () => {
  it('labels image 1 as a character sheet and folds extra sheets into the range', () => {
    const single = createIllustrationPrompt({ ...baseOpts, contentAnchor: 'sheet', characterSheetCount: 0, bridgeScene: scene });
    expect(single).toContain('image 1 is a CHARACTER SHEET');
    expect(single).toContain('it is NOT a scene to copy');

    const multi = createIllustrationPrompt({ ...baseOpts, contentAnchor: 'sheet', characterSheetCount: 2, bridgeScene: scene });
    expect(multi).toContain('images 1-3 are CHARACTER SHEETS');
  });

  it('emits the avatar scene section with the story-authored moment', () => {
    const prompt = createIllustrationPrompt({ ...baseOpts, contentAnchor: 'sheet', bridgeScene: scene });
  ...
  it('falls back to the page text when the scene failed validation', () => {
    const prompt = createIllustrationPrompt({ ...baseOpts, contentAnchor: 'sheet', bridgeScene: null });
    expect(prompt).toContain('DEPICT the moment this page\'s story text describes: "Drip, drop! Emma tiptoes to the gate."');
  });
...
describe('createIllustrationPrompt — interior anchor (avatar story covers)', () => {
  it('labels image 1 as the approved interior render and adds the cover-anchor section', () => {
    ...
    expect(prompt).toContain("image 1 is this book's approved interior illustration of this same scene");
