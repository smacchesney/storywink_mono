...
## 2026-07-15 — X11 Track E draw-again confirm (x11-e-drawagain)
...
## 2026-07-15 — X11 Track F redraw photo recovery (x11-f)

- `characters.drawAgainTryAgain` — the quiet "try again" action in the failed-recovery confirm dialog 「もういちど ためす」 ("Try again"); sits beside the primary `drawAgainFreshPhoto`. Deliberately distinct from `drawAgain` 「もういちど かく」 (which redraws) — this one just retries the same paid draw. Check ためす ("give it a go") reads as a low-stakes retry and doesn't collide in meaning with かく.
- `characters.drawAgainFreshPhoto` — the primary "add a fresh photo" recovery action on a FAILED target 「あたらしい しゃしんを つかう」 ("Add a fresh photo"); opens the single-photo picker. Check つかう ("use") vs ついか ("add") — `addPhotoCta` uses ついか; confirm the two verbs reading differently is fine (this one frames it as "use a new photo to draw", the needsPhoto one as "add a photo").
- `characters.needsPhotoBody` — the brick-case body when a redraw has no source 「{name} の しゃしんは、かいたあとに てばなしました。もういちど かくには、はっきりした しゃしんを 1まい ついかしてね。」 ("We let {name}'s photos go after drawing. Add one clear photo to draw {name} again."); echoes the retention promise wording in `photosHintBatch` (てばなす). Check てばなしました reads as the gentle "we let them go" retention promise (not data loss), and はっきりした しゃしん ("a clear photo") matches `photosHintBatch`'s うつりの いい しゃしん register.
- `characters.addPhotoCta` — the single "add a photo" action in the needsPhoto state 「しゃしんを ついか」 ("Add a photo"); the picker CTA. Check ついか ("add") is the right verb for the one-and-only action here and reads as a button.
- `characters.addingPhoto` — the working-copy label under the Storydust twinkle while the fresh photo uploads 「しゃしんを ついか しています…」 ("Adding the photo…"); check the 〜しています… ongoing register matches the app's other working copy (e.g. `styleDrawing` 「かいています…」) and the … reads as a wait.
- `characters.photoAddProblem` — the gentle toast when the upload or attach hiccups 「しゃしんを うけとれませんでした。もういちど おためしください。」 ("That photo didn't go through. Please try once more."); no system words, soft register like `createError`. Check うけとれませんでした ("couldn't receive it") avoids sounding like a hard failure and もういちど おためしください invites a calm retry.

## 2026-07-15 — X11 Track G shelf/wizard polish + cast caps removal (x11-g)

- `avatarStories.castHint` — the cast-step subtitle, retasked from a caps line to a selection invite 「このお話に出てくるみんなをえらんでね」 ("Choose everyone who's in this story"); doubles as the pick-someone requirement cue. Check えらんでね reads as a warm invite (not an instruction) and みんな covers people, pets and toys equally now that any mix is allowed.
- `avatarStories.castPickToBegin` — the single quiet helper shown near the disabled Next when zero characters are selected 「キャラクターをタップしてはじめよう」 ("Tap a character to begin"); check タップ / はじめよう register is friendly and not naggy, and that it does not overlap awkwardly with the castHint subtitle above the grid.
- `avatarStories.castFull` — the honest hint shown only once six are picked 「これでいっぱい — キャラクターは6つまでです」 ("That's a full cast — six characters at most"); replaces the retired per-kind caps (castPeopleFull/castCompanionsFull). Check the counter 6つ reads naturally for a mixed cast (people + pets + toys) and いっぱい lands as "that's plenty", not as a scolding limit.
- Retired keys (removed en+ja): `avatarStories.castPeopleFull`, `avatarStories.castCompanionsFull`, `avatarStories.castNeedsPerson` — the ≤4 people / ≤2 companions / ≥1 person rules are gone (owner decision "let the user decide"); no native review needed, listed for the record.

## 2026-07-15 — X11 Track H

- `exportDialog.readyBodyManual` — the ready-phase body shown when the PDF auto-save was skipped (iOS Safari with no live gesture) 「絵本ができあがりました。下の「PDFを保存」を押して、端末に保存してください。」 ("Your book is ready. Press Save PDF below to keep it on your device."); a sibling to the existing `readyBody`, which assumes the download already started. This one leads to the primary Save button instead of claiming a download is underway. Check the 〜てください register matches the app's other gentle CTAs and that 端末に保存 ("save to your device") reads naturally on iPhone without sounding technical.
- `characters.loading` — the screen-reader label on the shelf's first-load skeleton grid 「キャラクターを よみこんでいます…」 ("Loading your characters…"); the visible cue is the pulsing placeholder cards, so this is announced only, matching the app's ongoing 〜しています… working register (e.g. `addingPhoto` 「しゃしんを ついか しています…」). Check よみこんでいます reads naturally as "loading in" and the … conveys the wait.

## 2026-07-17 — X13 Track V ramble-ready spark (x13-track-v)

- `avatarStories.sparkPlaceholder` — the write-your-own field's placeholder, retasked from a question to a child-voiced ramble example 「くらやみがこわいドラゴンがいてね、マヤがナイトライトをさがしてあげるの…」 ("A dragon who's scared of the dark, and Maya helps him find a nightlight…"); it MODELS what a dictated child idea sounds like. Check the 〜てね／〜の sentence endings read as a child's storytelling voice a parent would recognise (not babytalk), and that the example name マヤ works as a placeholder child.
- `avatarStories.sparkHintCoCreate` — NEW, the quiet Geist hint under the spark textarea 「まよったら、つぎに なにが おこるか お子さんに きいてみて。そのまま はなしても かいても いいよ。」 ("Not sure? Ask your little one what happens next, then talk or type it here."). **NATIVE-REVIEW FLAG (reviewer): spacing** — this line uses spaced hiragana (the `characters`/`setup` convention) inside the otherwise unspaced `avatarStories` namespace; decide whether to unspace it to match its neighbours or accept the drift. Also check はなしても かいても いいよ conveys talk-or-type without naming any dictation feature.
- `avatarStories.sparkCharsLeft` — NEW, the near-cap counter shown only when fewer than 100 characters remain of the 1500 wall 「のこり{count}文字」 ("{count} characters left"). Check のこり{count}文字 reads naturally as a quiet countdown and the kanji 文字 is acceptable in this hiragana-lean namespace.
